# this code is only exporting one series
...
for series in json_data['Results']['series']:
    ...
    # df = pd.DataFrame(x._rows, columns=x.field_names)  # Use the rows directly to create the DataFrame
    # df.to_excel(seriesId + '.xlsx', index=False)

    # Save data to an Excel file
    df = pd.DataFrame([row.split() for row in x.get_string().split('\n') if row.strip()], columns=x.field_names)
    df.to_excel(seriesId + '.xlsx', index=False)
